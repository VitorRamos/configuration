...
-----------------------------------------------------------------------------------------------------------------------
local tagrunner = { taglist = {}, command = "", keys = {} }
...
tagrunner.keys.move = {
	{
		{}, "Down", function() tagrunner:down() end,
		{ description = "Select next item", group = "Navigation" }
	},
	{
		{}, "Up", function() tagrunner:up() end,
		{ description = "Select previous item", group = "Navigation" }
	},
}

tagrunner.keys.action = {
	{
...
tagrunner.keys.all = awful.util.table.join(tagrunner.keys.move, tagrunner.keys.action)
...
local function default_style()
	...
	return redutil.table.merge(style, redutil.table.check(beautiful, "float.tagrunner") or {})
end
...
local function list_filtrate(query)
	if lastquery ~= query then
		...
		tagrunner.taglist.position = 1
		tagrunner.taglist:update(programs.current)
		tagrunner.taglist:set_select(1)
		lastquery = query
	end
end
...
-----------------------------------------------------------------------------------------------------------------------
function tagrunner:down()
	if self.taglist.selected < math.min(self.itemnum, #programs.current) then
		self.taglist:set_select(self.taglist.selected + 1)
	elseif self.taglist.selected + self.taglist.position - 1 < #programs.current then
		self.taglist.position = self.taglist.position + 1
		self.taglist:update(programs.current)
	end
end

function tagrunner:up()
	if self.taglist.selected > 1 then
		self.taglist:set_select(self.taglist.selected - 1)
	elseif self.taglist.position > 1 then
		self.taglist.position = self.taglist.position - 1
		self.taglist:update(programs.current)
	end
end
...
local function keypressed_callback(mod, key)
	for _, k in ipairs(tagrunner.keys.all) do
		if redutil.key.match_prompt(k, mod, key) then k[3](); return true end
	end
	return false
end
local awful = require("awful")
-- Initialize tagrunner widget
-----------------------------------------------------------------------------------------------------------------------
function tagrunner:init()
	...
	programs.all = dfparser.tag_list(style.parser)
	...
	--------------------------------------------------------------------------------
	self.taglist = construct_list(tagrunner.itemnum, programs.current, style)
	...
	area_vertical:set_middle(wibox.container.margin(self.taglist.layout, 0, 0, style.border_margin[3]))
	local area_layout = wibox.container.margin(area_vertical, unpack(style.border_margin))

	-- Create floating wibox for tagrunner widget
	--------------------------------------------------------------------------------
	self.wibox = wibox({
		ontop        = true,
...
-- Show tagrunner widget
-- Wibox appears on call and hides after "enter" or "esc" pressed
-----------------------------------------------------------------------------------------------------------------------
function tagrunner:show()
	if not self.wibox then
		self:init()
	else
		list_filtrate("")
		self.taglist:set_select(1)
	end

	redutil.placement.centered(self.wibox, nil, mouse.screen.workarea)
	self.wibox.visible = true
	redtip:set_pack("tagrunner", self.keys.all, self.keytip.column, self.keytip.geometry)

	return awful.prompt.run({
		prompt = "",
		textbox = self.textbox,
		exe_callback = function () self.taglist.items[self.taglist.selected]:run() end,
...
-- Set user hotkeys
-----------------------------------------------------------------------------------------------------------------------
function tagrunner:set_keys(keys, layout)
